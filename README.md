...
Deploy `server.py` to Render or Railway as a Python web service. Set `PORT`, `STATIC_DIR` to the repo root, and `DATA_DIR` to a persistent disk path so `data.json` survives restarts.
